...
const SupportTicketTable = ({ 
  ...
  return (
    <div className="bg-white rounded-[16px] border border-[#EDEFF2] shadow-sm overflow-hidden flex flex-col min-h-[600px]">
      {/* Table Header with Search */}
      <div className="px-5 sm:px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h2 className="text-[20px] font-bold text-[#0C1824]">Support Ticket</h2>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#F1F5F9] bg-[#FAFBFC]">
              ...
              <th className="px-4 py-4 text-[13px] font-bold text-[#64748B] uppercase tracking-wider">Business Name</th>
              <th className="px-4 py-4 text-[13px] font-bold text-[#64748B] uppercase tracking-wider">Ticket ID</th>
              <th className="px-4 py-4 text-[13px] font-bold text-[#64748B] uppercase tracking-wider">Date</th>
              <th className="px-4 py-4 text-[13px] font-bold text-[#64748B] uppercase tracking-wider">Issues</th>
              <th className="px-4 py-4 text-[13px] font-bold text-[#64748B] uppercase tracking-wider">Message</th>
          ...
          <tbody className="divide-y divide-[#F1F5F9]">
            {tickets.map((ticket) => (
              <tr key={ticket.ticketId} className="hover:bg-[#F8FAFC] transition-colors group">
                <td className="px-8 py-5 text-[14px] font-medium text-[#64748B]">{ticket.id}</td>
                <td className="px-4 py-5 text-[14px] font-bold text-[#0C1824] whitespace-nowrap">{ticket.businessName}</td>
                <td className="px-4 py-5 text-[14px] font-medium text-[#374151] whitespace-nowrap">{ticket.ticketId}</td>
                <td className="px-4 py-5 text-[14px] font-medium text-[#374151] whitespace-nowrap">{ticket.date}</td>
                <td className="px-4 py-5 text-[14px] font-medium text-[#374151] whitespace-nowrap">{ticket.issues}</td>
                <td className="px-4 py-5 text-[14px] font-medium text-[#64748B] truncate max-w-[200px]">
                  {ticket.message}
                </td>
                <td className="px-4 py-5">
                  <span className={`inline-flex px-4 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase ${getStatusStyles(ticket.status)}`}>
                    {ticket.status}
                  </span>
                </td>
            ...
            {tickets.length === 0 && (
              <tr>
                <td colSpan={8} className="text-center py-20 text-[#64748B]">No tickets found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="px-4 sm:px-8 py-5 border-t border-[#F1F5F9] flex flex-col lg:flex-row items-center justify-between gap-5 bg-[#FAFBFC] mt-auto">
        <div className="text-[13px] sm:text-[14px] font-medium text-[#64748B] order-2 lg:order-1">
          Showing <span className="text-[#0C1824] font-bold">{(currentPage - 1) * itemsPerPage + (tickets.length > 0 ? 1 : 0)}</span> to <span className="text-[#0C1824] font-bold">{Math.min(currentPage * itemsPerPage, totalItems)}</span> of <span className="text-[#0C1824] font-bold">{totalItems}</span> entries
        </div>
